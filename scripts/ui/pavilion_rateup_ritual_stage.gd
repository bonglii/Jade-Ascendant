...
## Production ritual ornament for the locked Pavilion summon focal stage.
## Presentation-only: no economy, save, pity, Wish, or inventory mutation.
...
func _draw() -> void:
	...
	for star_index in range(STAR_POINTS.size()):
		...
		draw_circle(
			star_position,
			1.0 + float(star_index % 2),
			Color(star_tint.r, star_tint.g, star_tint.b, 0.15 + pulse * 0.04)
		)
