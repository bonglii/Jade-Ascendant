...
func _ready() -> void:
	...
	hero.add_theme_stylebox_override("panel", _make_card_style(Color(0.34, 0.90, 0.80), true))
	content.add_child(hero)
	var hero_box: VBoxContainer = VBoxContainer.new()
	hero_box.add_theme_constant_override("separation", 4)
	hero.add_child(hero_box)
	_add_label(hero_box, tr("ARCHIVE"), 10, Color(0.34, 0.90, 0.80))
	_add_label(hero_box, tr("Credits & Licenses"), 27, Color(0.98, 0.83, 0.43))
	_add_label(hero_box, tr("Jade Ascendant • Open-source notices and acknowledgements"), 12, Color(0.63, 0.74, 0.71))

	var card: PanelContainer = PanelContainer.new()
	card.add_theme_stylebox_override("panel", _make_card_style(Color(0.30, 0.75, 0.72), false))
	...
	credits.selection_enabled = true
	credits.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	credits.add_theme_font_size_override("normal_font_size", 13)
	credits.add_theme_color_override("default_color", Color(0.76, 0.84, 0.81))
	credits.text = "Jade Ascendant\n\n" + Engine.get_license_text() + "\n\n" + tr("THIRD-PARTY NOTICES") + "\n\n"
...
func _make_card_style(accent: Color, hero: bool) -> StyleBoxFlat:
	...
	style.bg_color = Color(0.002, 0.022, 0.034, 0.94 if hero else 0.90)
	style.border_width_left = 2 if hero else 1
	style.border_width_top = 1
	style.border_width_right = 1
	style.border_width_bottom = 1
	style.border_color = Color(accent.r, accent.g, accent.b, 0.58 if hero else 0.30)
...
func _add_label(parent_node: Node, text_value: String, font_size: int, tint: Color) -> Label:
...
func _back() -> void:
	...
	if change_error != OK:
		push_error("CreditsScreen: gagal kembali ke Privacy. Error code: " + str(change_error))
