...
## Crimson Moon Sect controller.
## Reuses the proven Stage 1 arena systems while owning Chapter 2 catalog,
## palette, pressure profile, autosave, and hazard cadence.
...
func _enter_tree() -> void:
	...
	get_node("WaveManager").set(
		"wave_duration",
		float(stage_profile["wave_duration"])
	)
...
func _ready() -> void:
	...
	var stage_label: Label = (
		get_node("HUD/ScreenRoot/HUDSafeArea/StageIdentity")
		as Label
	)
	stage_label.text = "2-%d  /  %s" % [
		stage_id,
		str(stage_profile["display_name"]).to_upper()
	]
	DebugLogger.system(
		"Stage 2-%d | %s" % [stage_id, stage_profile["display_name"]]
	)

func _apply_environment_profile() -> void:
	var ground: Polygon2D = (
		get_node("VerdantQiValleyEnvironment")
		as Polygon2D
	)
	var ground_material: ShaderMaterial = (
		ground.material.duplicate()
		as ShaderMaterial
	)
	ground.material = ground_material
	ground_material.set_shader_parameter(
		"deep_earth",
		stage_profile["ground"]
	)
	ground_material.set_shader_parameter(
		"moss_jade",
		stage_profile["moss"]
...
func _process(delta: float) -> void:
	...
	var start_wave: int = int(
		stage_profile.get("hazard_start_wave", 3)
	)
	if (
		wave_manager.current_wave < start_wave
		or wave_manager.current_wave >= wave_manager.final_wave
	):
		return
	...
	hazard_timer = float(
		stage_profile.get("hazard_interval", 10.0)
	)
	hazard_cycle += 1
	...
	var flank_wave: int = int(
		stage_profile.get("hazard_flank_wave", 999)
	)
	if wave_manager.current_wave >= flank_wave:
		var flank_offset: float = float(
			stage_profile.get("hazard_flank_offset", 145.0)
		)
		...
		_spawn_hazard(
			target + flank_direction * flank_offset,
			kind
		)
...
func _save_safe_checkpoint() -> void:
	if (
		not is_node_ready()
		or not JourneyManager.has_active_run()
